...
class Overdue2(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(Overdue2, self).__init__(cr, uid, name, context=context)
    ...
    def _lines_get(self, partner):
        ...
        movelines = moveline_obj.search(self.cr, self.uid,
                [('partner_id', '=', partner.id),
                    ('account_id.type', 'in', ['receivable']),#('account_id.code', 'in', ['120000','120200','121000']),
                    ('state', '<>', 'draft'), ('reconcile_id', '=', False),
                    '|', ('date_maturity','<=',time.strftime('%Y-%m-%d %H:%M:%S')), ('date_maturity', '=', False)
...
report_sxw.report_sxw('report.account.overdue.pekarna', 'res.partner', 'pekarna_reports/report/account_print_overdue.rml', parser=Overdue2)
