...
class Overdue(report_sxw.rml_parse):
    ...
    def _get_description(self, line):
        if line.journal_id.id == 15:
            return line.name
        else:
            return line.move_id.name

report_sxw.report_sxw('report.account.overdue.mentis', 'res.partner', 'mentis_reports/report/account_print_overdue.rml', parser=Overdue)
