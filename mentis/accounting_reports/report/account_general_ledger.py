...
class general_ledger(report_sxw.rml_parse, common_report_header):
    _name = 'report.account.general.ledger.mentis'
    ...
    def lines(self, account):
        ...
        if res_lines and self.init_balance:
            ...
            sql = """
                SELECT 0 AS lid, '' AS ldate, '' AS lcode, COALESCE(SUM(l.amount_currency),0.0) AS amount_currency, '' AS lref, 'Zacetno stanje' AS lname, COALESCE(SUM(l.debit),0.0) AS debit, COALESCE(SUM(l.credit),0.0) AS credit, '' AS lperiod_id, '' AS lpartner_id,
        ...
        for l in res:
            ...
            if l['move'] != '*':
                debit_initial += l['debit']
                credit_initial += l['credit']
                
            reference = l['lref']
            if l['lref'] and len(l['lref']) > 14 and (' ' not in l['lref']):
                new_string = l['lref'][:14] + ' ' + l['lref'][14:]
                l['lref'] = new_string
            
        if self.init_balance:
            res_init_sum = {}
            res_init_sum['move'] = '*'
            res_init_sum['lname'] = 'VSOTA PROMETA'
            res_init_sum['debit'] = debit_initial
            res_init_sum['credit'] = credit_initial
            res_init_sum['progress'] = 0
            res_init_sum['line_corresp'] = ''
            res_init_sum['lref'] = ''
            res_init_sum['partner_name'] = ''
            res_init_sum['lcode'] = ''
            res_init_sum['ldate'] = ''
            res_init_sum['period_code'] = ''
            res_init_sum['SUM'] = '1'
            list_init = [res_init_sum]
            res += list_init
...
report_sxw.report_sxw('report.account.general.ledger.mentis', 'account.account', 'addons/accounting_reports/report/account_general_ledger1.rml', parser=general_ledger, header='internal')
report_sxw.report_sxw('report.account.general.ledger_landscape.mentis', 'account.account', 'addons/accounting_reports/report/account_general_ledger_landscape1.rml', parser=general_ledger, header='internal landscape')
